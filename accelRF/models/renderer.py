'''
This part is made for NSVF renderer
Renderer takes samples, rayo, rayd, encoder status as input.
The field_fn->ImplicitField
'''
import math
...
class VolumeRenderer(Renderer):
    ...
    def __init__(self, args):
        ...
        self.discrete_reg = getattr(args, "discrete_regularization", False) # True
        self.raymarching_tolerance = getattr(args, "raymarching_tolerance", 0.0) # use for early stop
    ...
    def forward_chunk(
        ...
        tolerance = self.raymarching_tolerance # 0
...
if __name__=="__main__":
    ...
    test = VolumeRenderer(args)
    rayo = torch.zeros(10,10).cuda()
    rayd = torch.ones(10,10).cuda()
    ray(rayo,rayd,rayd)